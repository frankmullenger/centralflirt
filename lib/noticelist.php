...
class NoticeListItem extends Widget
{
    ...
    {
        //Discourage replying to users that are not subscribed to each other as this does not currently work
        if (common_config('profile', 'enable_dating')) {
            $this->showDatingReplyLink();
            return;
        }
    ...
    function showDatingReplyLink()
    {
        $user = common_current_user();
        if (!is_null($user) && $user->isSubscriber($this->profile) && $user->id != $this->profile->id) {
            $reply_url = common_local_url('newnotice',
                                          array('replyto' => $this->profile->nickname));
